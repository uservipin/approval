import json, pickle, time, warnings
from pathlib import Path
from typing import List, Tuple, Optional, Dict, Any, Union

import numpy as np
import pandas as pd
from sklearn.base import BaseEstimator, TransformerMixin
from sklearn.compose import ColumnTransformer
from sklearn.pipeline import Pipeline
from sklearn.impute import SimpleImputer
from sklearn.preprocessing import OneHotEncoder, RobustScaler
from sklearn.linear_model import LogisticRegression
from sklearn.model_selection import train_test_split, GridSearchCV, StratifiedKFold
from sklearn.metrics import (f1_score, precision_score, recall_score, roc_auc_score,
                             average_precision_score, classification_report, precision_recall_curve)

# ---------------- Config ----------------
DEFAULT_DATA = "model_resources/Loan_dataset_india.csv"
DEFAULT_TARGET = "credit_approved"
FEATURE_CLASSIFICATION: Optional[str] = "model_resources/feature_classification.csv"
OUT_DIR = "model_resources"

# speed/quality knobs
CV_FOLDS = 10
FAST_MODE = True        # if True -> only lbfgs; if False -> lbfgs & saga
MAX_ITER = 100
C_GRID = [10, 100, 200]

# threshold constraints
MIN_PRECISION = 0.72
MIN_RECALL = 0.65

RANDOM_STATE = 44
TEST_SIZE = 0.25

# ---------------- Utility helpers ----------------
def _make_serializable(obj: Any) -> Any:
    """Best-effort conversion of numpy/pandas types to JSON-serializable Python types."""
    if isinstance(obj, (np.floating, np.float32, np.float64)):
        return float(obj)
    if isinstance(obj, (np.integer,)):
        return int(obj)
    if isinstance(obj, (np.ndarray,)):
        return obj.tolist()
    if isinstance(obj, (pd.Series,)):
        return obj.to_list()
    if isinstance(obj, (pd.DataFrame,)):
        return obj.to_dict(orient="records")
    if obj is None:
        return None
    return obj

def _ohe_feature_names(ohe: OneHotEncoder, input_features: List[str]) -> List[str]:
    """Get expanded OHE output names in a version-safe way."""
    names: List[str] = []
    try:
        # Newer scikit-learn
        names = list(ohe.get_feature_names_out(input_features))
    except Exception:
        # Older scikit-learn: build manually from categories_
        try:
            cats = ohe.categories_
            for col_name, levels in zip(input_features, cats):
                for lvl in levels:
                    names.append(f"{col_name}_{lvl}")
        except Exception:
            names = []
    return names

# --------------- Feature tagging ------------------
def make_ohe():
    # Try modern API; fallback to legacy args for older sklearn versions
    try:
        return OneHotEncoder(handle_unknown="ignore", sparse_output=False, min_frequency=0.05)
    except TypeError:
        return OneHotEncoder(handle_unknown="ignore", sparse=False)

def read_feature_classification(csv_path: Optional[str]) -> Optional[pd.DataFrame]:
    if not csv_path:
        return None
    p = Path(csv_path)
    if not p.exists():
        return None
    df = pd.read_csv(p)
    colmap = {c.lower(): c for c in df.columns}
    feat = colmap.get("feature", list(df.columns)[0])
    cls  = colmap.get("classification", list(df.columns)[1])
    df = df.rename(columns={feat: "Feature", cls: "Classification"})
    df["Feature"] = df["Feature"].astype(str)
    df["Classification"] = (
        df["Classification"].astype(str).strip().str.lower()
        .map({"protected": "protected", "derived": "derived", "given": "given"})
        .fillna("given")
    )
    return df

def tag_features_from_classification(all_cols: List[str], fc_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    tag_map = {} if fc_df is None else dict(zip(fc_df["Feature"].astype(str), fc_df["Classification"]))
    rows = []
    for c in all_cols:
        t = tag_map.get(c, "given")
        tag = t if t in {"given", "derived", "protected"} else "given"
        rows.append({"feature": c, "tag": tag, "raw_tag": t})
    return pd.DataFrame(rows)

def apply_feature_tags_and_filter(df: pd.DataFrame, target: str, fc_path: Optional[str]):
    cols = [c for c in df.columns if c != target]
    tags = tag_features_from_classification(cols, read_feature_classification(fc_path))
    allowed = tags[tags["tag"].isin(["given", "derived"])]["feature"].tolist()
    protected = tags[tags["tag"] == "protected"]["feature"].tolist()
    out = pd.concat([df[allowed], df[target]], axis=1)
    tags["used_in_training"] = tags["feature"].isin(allowed)
    return out, tags, allowed, protected

# --------------- Preprocessing ------------------
class OutlierCapper(BaseEstimator, TransformerMixin):
    """
    Caps outliers column-wise:
    - If |skew| < 1: use mean ± 3*std (Gaussian-ish)
    - Else: use IQR rule [Q1 - 1.5*IQR, Q3 + 1.5*IQR]
    """
    def __init__(self):
        self.bounds_: List[Tuple[Optional[float], Optional[float]]] = []

    def fit(self, X, y=None):
        A = self._arr(X)
        self.bounds_ = []
        for i in range(A.shape[1]):
            col = A[:, i]
            mask = ~np.isnan(col)
            v = col[mask]
            if v.size == 0:
                self.bounds_.append((None, None))
                continue
            skew = pd.Series(v).skew()
            if abs(skew) < 1:
                m, s = float(np.mean(v)), float(np.std(v, ddof=0))
                low, high = (m, m) if (s == 0 or np.isnan(s)) else (m - 3 * s, m + 3 * s)
            else:
                q1, q3 = np.percentile(v, 25), np.percentile(v, 75)
                iqr = q3 - q1
                low, high = (q1, q3) if iqr == 0 else (q1 - 1.45 * iqr, q3 + 1.55 * iqr)
            self.bounds_.append((low, high))
        return self

    def transform(self, X):
        A = self._arr(X)
        B = A.copy()
        for i, (lo, hi) in enumerate(self.bounds_):
            if lo is not None and hi is not None:
                B[:, i] = np.clip(A[:, i], lo, hi)
        return B

    @staticmethod
    def _arr(X):
        return X.values.astype(float) if isinstance(X, pd.DataFrame) else np.asarray(X, dtype=float)

def infer_feature_types(df: pd.DataFrame, target: str):
    cols = [c for c in df.columns if c != target]
    num = df[cols].select_dtypes(include=[np.number]).columns.tolist()
    cat = [c for c in cols if c not in num]
    return num, cat

def build_preprocessor(num_cols: List[str], cat_cols: List[str]) -> ColumnTransformer:
    num_tf = Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("cap", OutlierCapper()),
        ("scale", RobustScaler())
    ])
    cat_tf = Pipeline([
        ("imputer", SimpleImputer(strategy="most_frequent")),
        ("ohe", make_ohe())
    ])
    return ColumnTransformer([
        ("num", num_tf, num_cols),
        ("cat", cat_tf, cat_cols)
    ], remainder="drop")

def build_pipeline(prep: ColumnTransformer) -> Pipeline:
    clf = LogisticRegression(
        penalty="l2",
        class_weight="balanced",
        max_iter=MAX_ITER,
        random_state=RANDOM_STATE
    )
    return Pipeline([("prep", prep), ("clf", clf)])

def get_param_grid() -> List[Dict[str, Any]]:
    solvers = ["lbfgs"] if FAST_MODE else ["lbfgs", "saga"]
    return [{
        "clf__solver": solvers,
        "clf__penalty": ["l2"],
        "clf__C": C_GRID,
        "clf__class_weight": ["balanced"],
        "clf__max_iter": [MAX_ITER]
    }]

# --------------- Thresholding & Metrics ------------------
def choose_threshold(y_true: Union[np.ndarray, pd.Series],
                    y_proba: np.ndarray,
                    min_p: float,
                    min_r: float) -> Dict[str, Any]:
    p, r, thr = precision_recall_curve(y_true, y_proba)
    cand = []
    for pi, ri, ti in zip(p[:-1], r[:-1], thr):
        if pi >= min_p and ri >= min_r:
            f1 = 2 * pi * ri / (pi + ri + 1e-12)
            cand.append((ti, pi, ri, f1))
    if cand:
        ti, pi, ri, f1 = max(cand, key=lambda x: x[3])
        return {"threshold": float(ti), "precision": float(pi), "recall": float(ri),
                "f1": float(f1), "constraints_met": True}

    # fallback to best F1
    f1s = [2 * pi * ri / (pi + ri + 1e-12) for pi, ri in zip(p[:-1], r[:-1])]
    bi = int(np.argmax(f1s)) if f1s else 0
    bt = float(thr[bi]) if len(thr) > 0 else 0.5
    return {"threshold": bt, "precision": float(p[bi]), "recall": float(r[bi]),
            "f1": float(f1s[bi]) if f1s else 0.0, "constraints_met": False}

def evaluate(y_true, y_pred, y_proba=None) -> Dict[str, Any]:
    m = {
        "f1": f1_score(y_true, y_pred, zero_division=0),
        "precision": precision_score(y_true, y_pred, zero_division=0),
        "recall": recall_score(y_true, y_pred, zero_division=0)
    }
    if y_proba is not None:
        try:
            m["roc_auc"] = roc_auc_score(y_true, y_proba)
        except Exception:
            m["roc_auc"] = None
        try:
            m["pr_auc"] = average_precision_score(y_true, y_proba)
        except Exception:
            m["pr_auc"] = None
    return m

# --------------- Audit extraction ------------------
def extract_preprocessing_artifacts(best: Pipeline,
                                    num_cols: List[str],
                                    cat_cols: List[str]) -> Dict[str, Any]:
    """Pull fitted preprocessing details from the trained pipeline for audit logs."""
    artifacts: Dict[str, Any] = {
        "numeric_features": list(num_cols),
        "categorical_features": list(cat_cols),
        "numeric": {},
        "categorical": {}
    }
    prep: ColumnTransformer = best.named_steps["prep"]

    # Resolve fitted sub-pipelines and columns
    num_pipe = None
    cat_pipe = None
    fitted_num_cols: List[str] = list(num_cols)
    fitted_cat_cols: List[str] = list(cat_cols)

    try:
        for name, trans, cols in prep.transformers_:
            if name == "num":
                num_pipe = trans
                fitted_num_cols = list(cols) if not isinstance(cols, slice) else list(num_cols)
            elif name == "cat":
                cat_pipe = trans
                fitted_cat_cols = list(cols) if not isinstance(cols, slice) else list(cat_cols)
    except Exception:
        pass

    # Numeric pipeline details
    if num_pipe is not None:
        try:
            imputer: SimpleImputer = num_pipe.named_steps.get("imputer")
            capper: OutlierCapper = num_pipe.named_steps.get("cap")
            scaler: RobustScaler = num_pipe.named_steps.get("scale")

            num_block: Dict[str, Any] = {
                "imputer": {
                    "strategy": getattr(imputer, "strategy", "median"),
                    "statistics_": _make_serializable(getattr(imputer, "statistics_", None))
                },
                "outlier_capper": {
                    "bounds_by_feature": [
                        {
                            "feature": col,
                            "low": _make_serializable(lo),
                            "high": _make_serializable(hi)
                        }
                        for col, (lo, hi) in zip(fitted_num_cols, getattr(capper, "bounds_", []))
                    ]
                },
                "robust_scaler": {
                    "with_centering": getattr(scaler, "with_centering", True),
                    "with_scaling": getattr(scaler, "with_scaling", True),
                    "quantile_range": _make_serializable(getattr(scaler, "quantile_range", (25.0, 75.0))),
                    "center_": _make_serializable(getattr(scaler, "center_", None)),
                    "scale_": _make_serializable(getattr(scaler, "scale_", None))
                }
            }
            artifacts["numeric"] = num_block
        except Exception as e:
            artifacts["numeric_error"] = f"{type(e).__name__}: {e}"

    # Categorical pipeline details
    if cat_pipe is not None:
        try:
            cimputer: SimpleImputer = cat_pipe.named_steps.get("imputer")
            ohe: OneHotEncoder = cat_pipe.named_steps.get("ohe")

            cats = []
            try:
                cats = [list(map(_make_serializable, c)) for c in getattr(ohe, "categories_", [])]
            except Exception:
                cats = []

            ohe_names = _ohe_feature_names(ohe, fitted_cat_cols)

            cat_block: Dict[str, Any] = {
                "imputer": {
                    "strategy": getattr(cimputer, "strategy", "most_frequent")
                },
                "one_hot_encoder": {
                    "handle_unknown": getattr(ohe, "handle_unknown", "ignore"),
                    "drop": _make_serializable(getattr(ohe, "drop", None)),
                    "min_frequency": _make_serializable(getattr(ohe, "min_frequency", None)),
                    "sparse_output": _make_serializable(getattr(ohe, "sparse_output", None))
                                   if hasattr(ohe, "sparse_output") else None,
                    "categories_per_feature": {
                        col: levels for col, levels in zip(fitted_cat_cols, cats)
                    },
                    "expanded_feature_names": ohe_names,
                    "expanded_feature_count": len(ohe_names)
                }
            }
            artifacts["categorical"] = cat_block
        except Exception as e:
            artifacts["categorical_error"] = f"{type(e).__name__}: {e}"

    return artifacts

# --------------- Main ------------------
def main(data_path=DEFAULT_DATA,
         target=DEFAULT_TARGET,
         feature_classification=FEATURE_CLASSIFICATION):

    warnings.filterwarnings("ignore", category=UserWarning)

    data_path = Path(data_path)
    assert data_path.exists(), f"Missing data: {data_path}"

    df = pd.read_csv(data_path)
    print("Shape of DF", df.shape)
    df.columns = [str(c).strip() for c in df.columns]
    if target not in df.columns:
        raise ValueError(f"Target '{target}' not in data.")

    # --- Target cleaning ---
    y_raw = df[target]
    if not pd.api.types.is_numeric_dtype(y_raw):
        mapping = {
            "y": 1, "yes": 1, "true": 1, "t": 1, "approved": 1, "1": 1,
            "n": 0, "no": 0, "false": 0, "f": 0, "rejected": 0, "0": 0
        }
        y_series = y_raw.astype(str).str.strip().str.lower().map(mapping)
    else:
        y_series = pd.to_numeric(y_raw, errors="coerce")

    mask = y_series.notna()
    dropped = int(len(y_series) - mask.sum())
    if dropped > 0:
        print(f"[warn] dropped {dropped} rows with invalid target")
    df = df.loc[mask].copy()
    y = y_series.loc[mask].astype(int)
    print("Dataframe after target cleaning:", df.shape)

    # --- Feature tags, protected filtering ---
    df_f, tags, allowed, protected = apply_feature_tags_and_filter(df, target, feature_classification)

    X = df_f.drop(columns=[target])
    y = y.reindex(df_f.index)  # align, just in case

    # Class distribution (overall)
    full_class_dist = pd.Series(y).value_counts().to_dict()

    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=y
    )

    # Class distribution (splits)
    train_class_dist = pd.Series(y_train).value_counts().to_dict()
    test_class_dist = pd.Series(y_test).value_counts().to_dict()

    # Infer types on train
    num, cat = infer_feature_types(X_train, target="")
    prep = build_preprocessor(num, cat)
    pipe = build_pipeline(prep)
    grid = get_param_grid()

    # Safe CV folds given minority class size
    min_class = int(pd.Series(y_train).value_counts().min())
    n_splits = max(2, min(CV_FOLDS, min_class))
    cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=RANDOM_STATE)

    combos = len(grid[0]["clf__solver"]) * len(grid[0]["clf__C"])
    print(f"[info] Effective grid combos: {combos} | CV folds: {n_splits} | total fits: {combos * n_splits}")

    # --- Train ---
    gs = GridSearchCV(pipe, param_grid=grid, cv=cv, scoring="f1", n_jobs=-1, verbose=1)
    gs.fit(X_train, y_train)
    best: Pipeline = gs.best_estimator_

    # --- Predict & Threshold ---
    y_proba = best.predict_proba(X_test)[:, 1] if hasattr(best, "predict_proba") else None
    y_pred = best.predict(X_test)
    tinfo = None
    if y_proba is not None:
        tinfo = choose_threshold(y_test, y_proba, MIN_PRECISION, MIN_RECALL)
        y_pred = (y_proba >= tinfo["threshold"]).astype(int)

    mets = evaluate(y_test, y_pred, y_proba)
    print("\n[report] Post-threshold classification report")
    print(classification_report(y_test, y_pred, zero_division=0))

    # --- Preprocessing audit extraction (from fitted best pipeline) ---
    prep_audit = extract_preprocessing_artifacts(best, num, cat)

    # --- Build artifacts dict ---
    tags_records = tags.to_dict(orient="records")
    tag_counts = tags["tag"].value_counts().to_dict()

    out: Dict[str, Any] = {
        "run_meta": {
            "timestamp": int(time.time()),
            "random_state": RANDOM_STATE,
            "fast_mode": FAST_MODE
        },
        "data_meta": {
            "data_path": str(data_path),
            "rows": int(df.shape[0]),
            "cols": int(df.shape[1]),
            "test_size": TEST_SIZE,
            "class_distribution": {
                "overall": {str(k): int(v) for k, v in full_class_dist.items()},
                "train": {str(k): int(v) for k, v in train_class_dist.items()},
                "test": {str(k): int(v) for k, v in test_class_dist.items()}
            }
        },
        "grid_cv": {
            "cv_folds": n_splits,
            "solvers": grid[0]["clf__solver"],
            "C_grid": grid[0]["clf__C"],
            "max_iter": MAX_ITER,
            "best_params": gs.best_params_,
            "best_score_mean_cv_f1": float(gs.best_score_)
        },
        "threshold_constraints": {
            "min_precision": MIN_PRECISION,
            "min_recall": MIN_RECALL,
            "tuning": _make_serializable(tinfo)
        },
        "test_metrics": {k: _make_serializable(v) for k, v in mets.items()},
        "features": {
            "protected_features": list(protected),
            "used_features": list(allowed),
            "allowed_features_count": int(len(allowed)),
            "tag_summary_counts": {k: int(v) for k, v in tag_counts.items()},
            "all_features_with_tags": tags_records
        },
        "preprocessing_audit": prep_audit
    }

    # --- Persist artifacts ---
    out_dir = Path(OUT_DIR)
    out_dir.mkdir(parents=True, exist_ok=True)

    # CSV for human inspection
    (out_dir / "feature_tags_fast.csv").write_text(tags.to_csv(index=False))

    # JSON artifacts (with safe serialization)
    with open(out_dir / "run_artifacts_fast.json", "w") as f:
        json.dump(out, f, indent=2, default=_make_serializable)

    # Trained model
    with open(out_dir / "best_model_fast.pkl", "wb") as f:
        pickle.dump(best, f)

    # --- Summary to console ---
    print("\n=== Summary ===")
    print(json.dumps(out, indent=2, default=_make_serializable))

    if tinfo and not tinfo.get("constraints_met", False):
        print(
            f"\n[warn] Could not meet precision>={MIN_PRECISION:.2f} & recall>={MIN_RECALL:.2f} at any threshold. "
            f"Best: P={tinfo['precision']:.3f}, R={tinfo['recall']:.3f}."
        )

if __name__ == "__main__":
    main()
